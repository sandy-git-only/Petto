...
const AdoptionListElement = () => {
    ...
    useEffect(() => {
        ...
        const fetchPostResults = async () => {
          try {
            const response = await axios.get(`${REACT_APP_BASE_URL}/pets/user-post?userID=${user}`);
      ...
      <Link to={`/pets/details/${pet.id}`}>
        ...
        <MessageContainer>
            <MessageCard style={{display:"flex", flexDirection:"column"}}>
                <MessageImg style={{width:"20px", height:"20px"}} src={pet.category=="送養"? AdoptSmall : pet.category=="走失"? LostImg : SearchImg} />
                <text style={{fontSize:"8px", color:"gray"}}>{pet.category=="送養"? "送養" : pet.category=="走失"? "走失" : "拾獲"} </text>
            </MessageCard>
            <MessageCard>
                <MessageImg src={ligationImg} isTransparent={!(pet.ligation=="1")} />
            </MessageCard>
            <MessageCard>
                <MessageImg src={bugImg} isTransparent={!(pet.anthel=="1")} />
            </MessageCard>
            <MessageCard>
                <MessageImg src={pet.vaccine=="1"? vaccineImg : unvaccinatedImg} isTransparent={!(pet.vaccine=="1")} />
            </MessageCard>
